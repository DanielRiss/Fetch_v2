...
helper_prompt = "You are playing the Game of 24. Given 4 numbers, you may use addition, subtraction, multiplication, and division to combine them to reach the target number of 24. You must use each input number exactly once and can use parentheses to define the order of operations. Provide a step-by-step solution leading to the final answer.\n\n"
data_path = "/gpfs/home6/dris/projects/Fetch/gameOf24/go24-benchmark.csv"
# Build a filename with the ID embedded
filename = f"{JOBID}_test_go24_beamsearch_merge_b{BUDGET}_t{TEMPERATURE}.pkl"
output_fpath = os.path.join(LOGDIR, filename)
policy_fpath = "xmu-nlp/Llama-3-8b-gsm8k" # TODO: update to Go24 model
...
def call_policy(question, path):
    ...
    prompt = f"{helper_prompt} Question: {question}\nAnswer:{path}"
    payload = {
        "prompt": prompt,
        "model": policy_fpath,
